...
{
    public class GenerateExcelPengeluaran
    {
        public GenerateExcelPengeluaran()
        {
        ...
        public byte[] GeneratePengeluaran(List<Pengeluaran_bulanan> data)
        {
            using (var package = new ExcelPackage())
            {
                ...
                var transformedData = data.Select((p, index) => new
                {
                    No = index + 1,
                    Nama = p.Pengeluaran_harian.Nama,
                    Tanggal = p.Pengeluaran_harian.Created_at,
                    Harga = p.Pengeluaran_harian.Harga
                }).ToList();
